...
TEST(TactileValueArray, auto_init_offset) {
	...
	ASSERT_DEATH(sensor.updateValues(values, offset+1),
	             ".*Assertion .* failed.");
	ASSERT_DEATH(sensor.updateValues(values, -(sensor.size()+1)),
	             ".*Assertion .* failed.");
}
...
TEST(TactileValueArray, getValues) {
	...
	ASSERT_DEATH(sensor.getValues(TactileValue::rawCurrent, expected),
	             ".*Assertion .* failed.");
...
TEST(TactileValueArray, accumulate) {
	TactileValueArray sensor;
	sensor.updateValues(values);
	std::map<TactileValueArray::AccMode, float> expected = {
	   {TactileValueArray::Sum, 0},
